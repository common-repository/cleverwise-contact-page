...
function cw_contact_page_mailout($cw_cp_to_email,$cw_email,$cw_cp_subject,$cw_cp_body,$cp_mail_server,$cp_mail_port,$cp_mail_conn_type,$cp_mail_email,$cp_mail_passwd) {
	if (!function_exists('PHPMailer')) {
		$cw_wp_includes_path=ABSPATH . WPINC;

		if (file_exists("$cw_wp_includes_path/PHPMailer/PHPMailer.php")) {
			require_once("$cw_wp_includes_path/PHPMailer/Exception.php");
			require_once("$cw_wp_includes_path/PHPMailer/SMTP.php");
			require_once("$cw_wp_includes_path/PHPMailer/PHPMailer.php");

			$cw_cp_mail_config = new PHPMailer\PHPMailer\PHPMailer();
		} else {
			require_once("$cw_wp_includes_path/class-smtp.php");
			require_once("$cw_wp_includes_path/class-phpmailer.php");
	...
	if ($cp_mail_conn_type == 'tls' or $cp_mail_conn_type == 'ssl') {
		...
		$cw_cp_mail_config->SMTPOptions = array('ssl' => array('verify_peer' => false,'verify_peer_name' => false,'allow_self_signed' => true));
	}
